...
- Задачи ring 3 через `syscall`/`sysret`; сисколлы: `log`, `exit`, `yield`,
  `sleep`, `send`, `recv`, `info`. Все регистры, кроме `rax/rcx/r11`,
...
- Сервисы в образе: `kbd` — драйвер PS/2-клавиатуры в ring 3 (ядро лишь
  пересылает сканкоды в эндпоинт), `hello`, `ping`/`pong` через IPC-эндпоинты
  и `flaky`, который на каждой третьей итерации разыменовывает NULL и
  поднимается обратно без перезагрузки.
...
- Капабилити на объекты памяти и shared-memory IPC для больших данных,
  передача капабилити через эндпоинты, асинхронные уведомления.
- Больше драйверов вне ядра: перечисление PCI, AHCI/virtio, VFS-сервер,
  доставка IRQ драйверам в ring 3 через эндпоинты.
- Сисколлы деривации/отзыва капабилити; аллокатор для `k1k-rt`.

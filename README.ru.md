...
- Задачи ring 3 через `syscall`/`sysret`; сисколлы: `log`, `exit`, `yield`,
  ...
  `mem_map`. Все регистры, кроме `rax/rcx/r11`, сохраняются через сисколл.
- Перечисление PCI-устройств через configuration space при загрузке (сами
  драйверы будут жить в ring 3).
...
- Сервисы в образе: `kbd` — драйвер PS/2-клавиатуры в ring 3 (ядро лишь
  пересылает сканкоды в эндпоинт), `hello`, `ping`/`pong` — запрос/ответ через
  эндпоинты плюс разделяемая страница, которую `pong` выделяет и отдаёт `ping`
  как капабилити, — и `flaky`, который на каждой третьей итерации
  разыменовывает NULL и поднимается обратно без перезагрузки.
...
- Часы HPET/TSC, межпроцессорные прерывания (TLB shootdown, удалённое
  перепланирование).
- Асинхронные уведомления; капабилити на IRQ и PCI-устройства, чтобы драйверы
  в ring 3 владели железом; драйверы AHCI/virtio и VFS-сервер.
